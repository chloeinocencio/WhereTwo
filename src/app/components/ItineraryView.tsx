...
export function ItineraryView({ session, itineraryId, onBack }: ItineraryViewProps) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`flight-${itineraryId}`);
    if (saved) {
      const parsed = JSON.parse(saved);
      setFlightDetails(parsed);
      setFlightForm(parsed);
    }
  }, [itineraryId]);

  const saveFlightDetails = () => {
    localStorage.setItem(`flight-${itineraryId}`, JSON.stringify(flightForm));
    setFlightDetails({ ...flightForm });
        ...
        <Card className="mb-8 border-2">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {flightDetails && !editingFlight && (
                  <Button variant="ghost" size="sm" onClick={clearFlightDetails} className="text-muted-foreground hover:text-destructive text-xs">
                    Remove
                  </Button>
                )}
                <Button variant="outline" size="sm" onClick={() => setEditingFlight(!editingFlight)}>
                  {editingFlight ? 'Cancel' : flightDetails ? 'Edit' : 'Add Flight'}
                </Button>
              </div>
            </div>
          </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                {(flightDetails.airline || flightDetails.flightNumber) && (
                  <div>
                    ...
                    <p className="font-semibold">{[flightDetails.airline, flightDetails.flightNumber].filter(Boolean).join(' · ')}</p>
                  </div>
                )}
                {flightDetails.confirmationCode && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-0.5">Confirmation</p>
                    <p className="font-semibold font-mono">{flightDetails.confirmationCode}</p>
                  </div>
                )}
                {flightDetails.departureAirport && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-0.5">Departure</p>
                    <p className="font-semibold">{flightDetails.departureAirport}</p>
                    {flightDetails.departureTime && <p className="text-xs text-muted-foreground">{flightDetails.departureTime}</p>}
                  </div>
                )}
                {flightDetails.arrivalAirport && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-0.5">Arrival</p>
                    <p className="font-semibold">{flightDetails.arrivalAirport}</p>
                    {flightDetails.arrivalTime && <p className="text-xs text-muted-foreground">{flightDetails.arrivalTime}</p>}
                  </div>
                )}
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">No flight details added yet. Click "Add Flight" to save your flight information.</p>
            )}
          </CardContent>
        </Card>
